...
class API2Server {
    ...
    setupMiddleware() {
        ...
        const limiter = rateLimit({
            ...
            legacyHeaders: false,
            validate: false // Disable validation warnings for proxy headers
        });
    ...
    async initializeServices() {
        try {
            ...
            // Initialize blockchain service with retry
            let retries = 0;
            const maxRetries = 3;
            ...
            while (retries < maxRetries) {
                try {
                    ...
                    if (retries === maxRetries) {
                        logger.error('Failed to initialize blockchain service after retries', { 
                            error: error.message,
                            retries 
                        });
                        // Don't throw, allow server to start in degraded mode
                        logger.api('Server starting in degraded mode - blockchain service unavailable');
                        break;
                    }
                    
                    // Wait before retry with exponential backoff
                    const delay = Math.min(1000 * Math.pow(2, retries), 10000);
                    logger.api(`Retrying blockchain initialization in ${delay}ms... (attempt ${retries + 1}/${maxRetries})`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
            }

            // Skip cache warmup on startup to avoid rate limiting
            // Cache will be populated on-demand
            logger.api('Cache will be populated on-demand to avoid rate limiting');

            logger.api('Service initialization completed');
        } catch (error) {
            logger.error('Service initialization failed', { error: error.message });
            // Don't throw - allow server to start
            logger.api('Server starting with limited functionality');
        }
    }
