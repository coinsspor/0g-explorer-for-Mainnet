...
export function RPCMonitoring() {
  ...
  const fetchRPCs = async () => {
    try {
      // API endpoint kullan - nginx/vite sorunlarını bypass eder
      const response = await fetch('/api/rpc-scanner?t=' + Date.now());
      ...
      // Fallback: static dosyayı dene
      try {
        const fallbackResponse = await fetch('/rpc_data.json?t=' + Date.now());
        if (fallbackResponse.ok) {
          const fallbackData = await fallbackResponse.json();
          processRPCData(fallbackData);
        }
      } catch (fallbackError) {
  ...
  const processRPCData = (data: any) => {
    ...
    setStats({
      ...
      good: good.length,
      slow: slow.length,
      avgLatency: data.rpcs.length > 0 ? Math.round(data.rpcs.reduce((a: number, b: RPC) => a + b.latency, 0) / data.rpcs.length) : 0
    });
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">0G Mainnet - {stats.total} Active EVM Endpoints</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="text-sm">
              ...
              <p className="text-muted-foreground">
                ...
                It recursively scans peer connections to find EVM-compatible nodes (ports 8545-65545) and verifies chain ID (0x4115). 
