...
export function StakeModal({ validator, onClose }: StakeModalProps) {
  ...
  const handleStake = async () => {
    ...
    try {
      ...
      if (error.code === 4001) {
        ...
        toast.error('Transaction failed - please check your balance and gas');
  ...
  const handleUnstake = async () => {
    ...
    try {
      ...
      const sharesInWei = BigInt(Math.floor(parseFloat(unstakeShares) * 1e18));
      
      const undelegateMethodId = '0x4d99dd16';
      const withdrawalAddressParam = walletAddress.toLowerCase().slice(2).padStart(64, '0');
      const sharesParam = sharesInWei.toString(16).padStart(64, '0');
      ...
      console.log('Unstake transaction params:', {
        ...txParams,
        amountInOG: unstakeShares,
        amountInWei: sharesInWei.toString()
      });
      ...
      if (error.code === 4001) {
        ...
        toast.error('Transaction failed - please check your staked balance');
      } else {
