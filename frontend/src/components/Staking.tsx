...
export function Staking() {
  ...
  const handleStake = async () => {
    ...
    try {
      ...
      if (chainId !== '0x4115') {
        toast.error('Please switch to 0G Mainnet (Chain ID: 16661)');
        return;
      }

      // Wei'ye çevirme - BigInt kullan
      const amountInWei = BigInt(Math.floor(parseFloat(stakeAmount) * 1e18));
      
      // Method signature ve parametreler
      const functionSignature = '0x5c19a95c'; // delegate(address)
      const delegatorParam = walletAddress.toLowerCase().slice(2).padStart(64, '0');
      const callData = functionSignature + delegatorParam;
      
      // Transaction parametreleri
      const txParams = {
        from: walletAddress,
        to: selectedValidator, // Validator adresini olduğu gibi kullan
      ...
      if (error.code === 4001) {
        ...
        // Internal JSON-RPC error - daha spesifik kontrol
        console.error('RPC Error - Check validator address:', selectedValidator);
        toast.error('Transaction failed - please check validator and try again');
  ...
  const handleUnstake = async () => {
    ...
    try {
      ...
      const chainId = await window.ethereum.request({ method: 'eth_chainId' });
      if (chainId !== '0x4115') {
        toast.error('Please switch to 0G Mainnet');
        return;
      }

      const withdrawalFee = BigInt(1000000000); // 1 Gwei
      const amountInWei = BigInt(Math.floor(parseFloat(unstakeAmount) * 1e18));
      
      const undelegateMethodId = '0x4d99dd16';
      const withdrawalAddressParam = walletAddress.toLowerCase().slice(2).padStart(64, '0');
      ...
      const txParams = {
        ...
        value: '0x' + withdrawalFee.toString(16),
        data: callData
      };
