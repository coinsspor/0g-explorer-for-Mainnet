...
export function Transactions({ onViewDetails, onViewWallet }: TransactionsProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [searchType, setSearchType] = useState<'auto' | 'tx' | 'wallet'>('auto');
  ...
  const detectSearchType = (input: string): 'tx' | 'wallet' | 'block' | 'unknown' => {
    if (input.startsWith('0x')) {
      // Transaction hash: 66 characters
      if (input.length === 66) return 'tx';
      // Wallet address: 42 characters
      if (input.length === 42) return 'wallet';
    }
    // Block number
    if (/^\d+$/.test(input)) return 'block';
  ...
  const getSearchPlaceholder = () => {
    switch (searchType) {
      ...
      default:
        return 'Enter tx hash, wallet address, or block number...';
    }
  };
    ...
    try {
      ...
      if (searchType === 'auto') {
        ...
        if (detectedType === 'unknown') {
          alert('Invalid search query. Please enter a valid transaction hash, wallet address, or block number.');
          setSearching(false);
          return;
        }
      }
      ...
      if (data.success) {
        if (data.type === 'transaction' && onViewDetails) {
          onViewDetails(data.data.hash);
        } else if (data.type === 'wallet' && onViewWallet) {
          onViewWallet(data.data.address);
        } else if (data.type === 'block') {
          // Handle block navigation if needed
          alert(`Block #${data.data.number} found. Hash: ${data.data.hash.slice(0, 10)}...`);
        }
      } else {
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={searchType} onValueChange={(value: any) => setSearchType(value)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="tx">Tx Hash</SelectItem>
                  ...
                  <TableRow key={tx.hash} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="font-mono text-sm p-0 h-auto hover:underline"
                        onClick={() => handleViewWallet(tx.fullFrom || tx.from)}
                      >
                        {tx.from}
                      </Button>
                    </TableCell>
                    <TableCell>
                      <Button
                        ...
                        size="sm"
                        className="font-mono text-sm p-0 h-auto hover:underline"
                        onClick={() => handleViewWallet(tx.fullTo || tx.to)}
                      >
                        {tx.to}
                      </Button>
                    </TableCell>
